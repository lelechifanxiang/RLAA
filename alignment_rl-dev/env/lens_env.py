...
_OPTICS_CORE_PATH = Path(
    os.environ.get(
        ...
        str(Path(__file__).parent.parent.parent / "optics_core-dev"),
    )
)
    ...
    def _ensure_core_prepared(self) -> None:
        ...
        if not self._core_prepared or self._core_prepare_dirty:
            ...
            try:
                ...
                # optics_core-dev00 predates incremental prepare() keywords.
...
class LensAlignmentEnv(gym.Env):
    ...
    def _sample_init_state(self) -> np.ndarray:
        ...
        强制采样远离零位的状态：对于每个DOF，以50%概率采样正半轴或负半轴的远端。
        采样范围：[0.5*rng, rng] 或 [-rng, -0.5*rng]，避免落在接近零位的 [-0.5, 0.5]*rng 区域。
        """
        state = []
        for lg in self.cfg.lens_groups:
            for d in lg.active_dofs:
                ...
                if self._np_random.random() < 0.5:
                    # 正半轴远端：[0.5*rng, rng]
                    val = self._np_random.uniform(0.5 * rng, rng)
                else:
                    # 负半轴远端：[-rng, -0.5*rng]
                    val = self._np_random.uniform(-rng, -0.5 * rng)
                state.append(val)
